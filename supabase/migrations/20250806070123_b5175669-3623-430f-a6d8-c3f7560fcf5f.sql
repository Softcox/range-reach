...
CREATE POLICY "Admins can manage identifiers" ON public.identifiers
  FOR INSERT USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Admins can manage opening balances" ON public.opening_balances
  FOR ALL USING (public.has_role(auth.uid(), 'admin'));
